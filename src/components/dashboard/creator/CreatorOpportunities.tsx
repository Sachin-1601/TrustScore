      ...
      {displayCampaigns.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {displayCampaigns.map((camp) => (
            <div
              key={camp.id}
              className="p-5 bg-slate-950/80 hover:bg-slate-950 border border-slate-800/90 hover:border-slate-700/80 rounded-2xl transition-all flex flex-col justify-between space-y-4"
            >
              <div className="space-y-2.5">
                ...
                <div>
                  <h4 className="font-bold text-slate-100 text-sm leading-snug line-clamp-1">
                    {camp.title}
                  </h4>
                ...
                <div className="space-y-1 pt-1 text-[11px] text-slate-400">
                  <div className="flex items-center justify-between">
                    ...
                    <strong className="text-slate-200">{camp.targetMinTrustScore || 80}+</strong>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Audience Range:</span>
                    <strong className="text-slate-200">{camp.targetFollowerRange || "Any"}</strong>
                  </div>
                </div>

                <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed bg-slate-900/60 p-2 rounded-xl border border-slate-800/60">
              ...
              <Link
                ...
                className="w-full py-2 px-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
              >
        ...
        <div className="p-8 bg-slate-950/80 rounded-2xl border border-slate-800 text-center space-y-3">
          ...
          <div className="pt-1">
            <Link
              ...
              className="inline-flex items-center gap-1.5 py-2 px-4 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 font-bold text-xs rounded-xl transition-all"
            >
